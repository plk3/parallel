...
func TestSearchFilesWithDepth(t *testing.T) {
	t.Run("normal case", func(t *testing.T) {
		// 一時ディレクトリを作成 (テスト終了時に自動で削除される)
		tempDir := t.TempDir()
		files := []string{
		...
		// テスト対象の関数を呼び出し
		foundFiles, err := SearchFilesWithDepth(tempDir, 0, func(path string) bool {
			return strings.Contains(path, "keyword")
		})
		if err != nil {
			t.Fatalf("SearchFiles failed: %v", err)
	...
	t.Run("invalid directory", func(t *testing.T) {
		// 存在しないディレクトリを指定
		_, err := SearchFilesWithDepth("/invalid/path", 0, func(path string) bool {
			return strings.Contains(path, "keyword")
		})
		if err == nil {
			t.Fatalf("Expected error, but got nil")
